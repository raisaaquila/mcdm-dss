...
def saw(matrix, weights, types, targets):
    ...
    for j in range(matrix.shape[1]):
        ...
        if typ == "benefit":
            ...
            min_val = col.min()
            normalized[:, j] = min_val / col if min_val != 0 else 0

        elif typ == "match":
            # Use target if provided, else fallback to max for normalization
            if t is None or t == 0:
                t = col.max()
            normalized[:, j] = 1 - np.abs(col - t) / t
            # Clip to [0,1] to avoid negatives if any
            normalized[:, j] = np.clip(normalized[:, j], 0, 1)

        else:
            # default to benefit if unknown type
            max_val = col.max()
            normalized[:, j] = col / max_val if max_val != 0 else 0
